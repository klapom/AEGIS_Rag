...
async def test_memory_search_endpoint_e2e(test_client_async, redis_memory_manager):
    ...
    await redis_memory_manager.store_conversation_context(
        session_id=session_id, messages=[{"role": "user", "content": "API test query"}]
    )
    response = await test_client_async.post(
        "/api/v1/memory/search", json={"query": "API test", "session_id": session_id, "top_k": 5}
    )
...
async def test_memory_search_with_layers_e2e(test_client_async):
    response = await test_client_async.post(
        ...
        json={"query": "test", "layers": ["short_term", "episodic"], "top_k": 3},
    )
    assert response.status_code == 200
...
async def test_memory_search_time_window_e2e(test_client_async):
    response = await test_client_async.post(
        "/api/v1/memory/search", json={"query": "test", "time_window_hours": 24, "top_k": 5}
    )
    assert response.status_code == 200
...
async def test_point_in_time_endpoint_e2e(test_client_async, neo4j_driver):
    ...
    response = await test_client_async.post(
        "/api/v1/memory/temporal/point-in-time",
        json={"query": "test_entity", "timestamp": timestamp, "top_k": 5},
    )
    assert response.status_code == 200
    data = response.json()
    assert "query" in data
...
async def test_point_in_time_validation_e2e(test_client_async):
    response = await test_client_async.post(
        "/api/v1/memory/temporal/point-in-time",
        json={"query": "test", "timestamp": "invalid_timestamp"},
    )
    assert response.status_code in [400, 422]
...
async def test_consolidate_endpoint_e2e(test_client_async):
    response = await test_client_async.post(
        ...
        json={"consolidate_to_qdrant": True, "consolidate_conversations": False},
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_consolidate_with_sessions_e2e(test_client_async, redis_memory_manager):
    ...
    await redis_memory_manager.store_conversation_context(
        session_id=session_id, messages=[{"role": "user", "content": "Consolidate this"}]
    )
    response = await test_client_async.post(
        "/api/v1/memory/consolidate",
        json={"consolidate_conversations": True, "active_sessions": [session_id]},
    )
    assert response.status_code == 200
...
async def test_session_delete_endpoint_e2e(test_client_async, redis_memory_manager):
    ...
    await redis_memory_manager.store_conversation_context(
        session_id=session_id, messages=[{"role": "user", "content": "Delete me"}]
    )
...
async def test_api_latency_targets_e2e(test_client_async):
    start = time.time()
    response = await test_client_async.post(
        "/api/v1/memory/search", json={"query": "latency test", "top_k": 5}
    )
