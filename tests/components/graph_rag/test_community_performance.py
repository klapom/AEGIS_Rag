...
class TestLargeGraphPerformance:
    ...
    async def test_cached_performance(self, detector, mock_neo4j_client, mock_graph_data):
        ...
        # Cached query should be fast (< 1000ms allows for CI system load, vs seconds for real query)
        assert cached_time_ms < 1000, f"Cached query took {cached_time_ms:.0f}ms, expected < 1000ms"

        # Verify cache was hit
        cache_info = detector.get_cache_info()
        assert cache_info["hits"] > 0, "Cache should have hits"

        print(f"\n[PASS] Cached query: {cached_time_ms:.0f}ms (target: < 1000ms)")
