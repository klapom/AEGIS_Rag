...
"""
    ...
    async def test_complete_document_ingestion_workflow(
        ...
        qdrant_count_before = await qdrant_client.count(collection_name=settings.qdrant_collection)
        ...
        print(
            f"✓ Baseline: {initial_chunk_count} chunks, {initial_entity_count} entities, {initial_relation_count} relations"
        )
        ...
        success_indicator = page.locator(
            'text=/upload.*success/i, text=/completed/i, [role="alert"]:has-text("success")'
        )
        ...
        qdrant_count_after = await qdrant_client.count(collection_name=settings.qdrant_collection)
        ...
        if search_result[0]:
            ...
            print(
                f"✓ Chunk structure validated: chunk_id={sample_chunk.payload['chunk_id'][:20]}..."
            )
        ...
        bm25_response = await page.request.post(
            ...
            data={"query": "machine learning", "top_k": 5, "search_type": "bm25"},
        )
        ...
        if citation_count > 0:
            ...
            if await citation_panel.count() > 0:
                ...
                assert any(
                    kw.lower() in citation_text.lower()
                    for kw in ["machine learning", "stanford", "google brain"]
