...
import { Search, Filter, ChevronLeft, ChevronRight, Download, Database, HardDrive, Network, Trash2 } from 'lucide-react';
 ...
 */
function SearchResultRow({ result, onDelete }: { result: MemorySearchResult; onDelete?: (id: string) => void }) {
...
export function MemorySearchPanel({ namespaces = [] }: MemorySearchPanelProps) {
  ...
  const handleDelete = useCallback(async (memoryId: string) => {
    // TODO: Implement memory deletion via API when backend endpoint is available
    ...
    <div className="space-y-4" data-testid="memory-search-panel">
      ...
      {hasSearched && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700">
          ...
          {/* Results List */}
          <div className="px-6 divide-y divide-gray-200 dark:divide-gray-700" data-testid="memory-list">
            {loading ? (
              ...
              results.map((result) => <SearchResultRow key={result.id} result={result} onDelete={handleDelete} />)
            )}
          </div>
