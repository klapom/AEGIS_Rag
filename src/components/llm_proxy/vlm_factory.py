...
Sprint Context: Sprint 36 (2025-12-05) - Feature 36.1: VLM Factory Pattern
...
Example:
    >>> from src.components.llm_proxy.vlm_factory import get_vlm_client
...
Example (Singleton Pattern):
    >>> from src.components.llm_proxy.vlm_factory import get_shared_vlm_client
...
See Also:
    - src/components/llm_proxy/ollama_vlm.py: Local Ollama backend
    - src/components/llm_proxy/dashscope_vlm.py: Cloud DashScope backend
...
from src.components.llm_proxy.vlm_protocol import VLMClient
...
def get_vlm_client(backend: VLMBackend | None = None) -> VLMClient:
    ...
    if backend == VLMBackend.OLLAMA:
        ...
        raise ValueError(
            f"Invalid VLM backend: {backend}. "
            f"Must be VLMBackend.OLLAMA or VLMBackend.DASHSCOPE"
        )
...
def reset_vlm_client() -> None:
    ...
    Example:
        >>> from src.components.llm_proxy.vlm_factory import reset_vlm_client
        >>> reset_vlm_client()
