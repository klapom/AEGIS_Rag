...
async def graph_extraction_node(state: IngestionState) -> IngestionState:
    ...
    try:
        ...
        # Sprint 41 FIX: Query Neo4j for actual chunks instead of using lightrag_docs IDs
        # The chunk_ids in lightrag_docs (from embedded_chunk_ids/Qdrant) don't match
        # the chunk_ids stored in Neo4j (generated by _chunk_text_with_metadata())
        document_id = state["document_id"]
        chunks_query = """
