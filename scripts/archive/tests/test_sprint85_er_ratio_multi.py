#!/usr/bin/env python3
"""Sprint 85 ER Ratio Test - Multi-Dataset Version.

Tests entity/relationship extraction across multiple RAGAS datasets:
1. ragas_eval_txt_large - Larger HotpotQA samples (5-14KB)
2. amnesty_qa_contexts - Amnesty International QA
3. ragas_phase1_contexts - Mixed: HotpotQA + LogQA + RAGBench

Usage:
    poetry run python scripts/test_sprint85_er_ratio_multi.py
"""
...
DATASETS = {
    "hotpot_large": {
        "path": Path("data/ragas_eval_txt_large"),
        "pattern": "sample_*.txt",
        "description": "HotpotQA Large (5-14KB per file)",
        "sample_size": 5,
    },
    "amnesty_qa": {
        "path": Path("data/amnesty_qa_contexts/contexts"),
        "pattern": "amnesty_context_*.txt",
        "description": "Amnesty QA Contexts",
        "sample_size": 5,
    },
    "ragas_phase1": {
        "path": Path("data/ragas_phase1_contexts"),
        "pattern": "ragas_phase1_*.txt",
        "description": "RAGBench + LogQA + HotpotQA Mixed",
        "sample_size": 5,
    },
}
...
async def upload_file(
    ...
    with open(file_path, "rb") as f:
        ...
        data = {"namespace": namespace, "domain": "ragas_test"}
...
async def test_dataset(
    ...
    # Find files
    data_path = Path("/home/admin/projects/aegisrag/AEGIS_Rag") / config["path"]
    ...
    namespace = f"er_test_{dataset_name}_{int(time.time())}"
    ...
    if overall_er >= 1.0:
        print(f"  ✅ TARGET ACHIEVED (ER >= 1.0)")
    else:
        print(f"  ⚠️ Below target (ER < 1.0)")
...
async def main():
    """Main test runner."""
    print("=" * 60)
    print("Sprint 85 ER Ratio Test - Multi-Dataset")
    print("=" * 60)
    ...
    async with httpx.AsyncClient() as client:
        ...
        print("=" * 60)
        print(f"{'Dataset':<25} {'Entities':>10} {'Relations':>10} {'ER Ratio':>10}")
        print("-" * 60)
        ...
        print("-" * 60)
        print(f"{'OVERALL':<25} {total_e:>10} {total_r:>10} {overall:>9.2f}")
        print("=" * 60)
