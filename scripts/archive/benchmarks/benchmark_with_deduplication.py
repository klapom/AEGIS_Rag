...
async def benchmark_with_deduplication(
    ...
    for i, chunk in enumerate(chunks):
        ...
        print(
            f"      Chunk {i + 1}/{len(chunks)}: {len(result.entities)} ent, "
    ...
    return {
        ...
        "dedup_reduction_pct": round(100 * (1 - multi_unique_count / simple_unique_count), 1)
        if simple_unique_count > 0
        else 0,
...
async def main():
    ...
    for r in all_results:
        ...
        print(
            f"{sample_id:<20} {r['raw_entities']:<8} {r['simple_dedup_entities']:<10} "
