    ...
    try:
        response = client.chat(
            ...
            messages=[
                ...
                {"role": "user", "content": ENTITY_USER_PROMPT_TEMPLATE.format(text=test_case["text"])}
            ],
...
def benchmark_model(model_name: str, client: Client) -> dict[str, Any]:
    ...
    for i, test_case in enumerate(TEST_CASES, 1):
        print(f"[{i}/10] Test {test_case['id']} ({test_case['complexity']})... ", end='', flush=True)
...
Complexity levels: SIMPLE, MEDIUM, COMPLEX, VERY COMPLEX
    ...
    # Avg time per test
    print(f"{'Avg Time per Test (s)':<30} {q8_result['avg_time_per_test']:<20.1f} {q4_result['avg_time_per_test']:<20.1f}")

    # Success rate
    print(f"{'Success Rate (%)':<30} {q8_result['success_rate']:<20.1f} {q4_result['success_rate']:<20.1f}")
    ...
    print(f"{'Test':<8} {'Complexity':<15} {'Q8_0 Ent':<12} {'Q4_K_M Ent':<12} {'Q8_0 Time':<12} {'Q4_K_M Time'}")
    ...
    for i in range(len(TEST_CASES)):
        ...
        print(f"{q8_test['test_id']:<8} {q8_test['complexity']:<15} "
    ...
    print(f"Speed:     Q4_K_M is {abs(speed_improvement):.1f}% {'FASTER' if speed_improvement > 0 else 'SLOWER'}")
    print(f"Quality:   Q4_K_M extracts {abs(quality_diff_pct):.1f}% {'MORE' if quality_diff_pct > 0 else 'FEWER'} entities")
    print(f"Success:   Q8_0={q8_result['success_count']}/10, Q4_K_M={q4_result['success_count']}/10")
