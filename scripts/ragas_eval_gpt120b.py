...
async def main():
    ...
    try:
        ...
        print()
        print(
            f"⏱️  Duration: {results.duration_seconds:.1f} seconds ({results.duration_seconds / 60:.1f} minutes)"
        )
        ...
        if results.per_intent_metrics:
            ...
            for intent_metrics in results.per_intent_metrics:
                ...
                print(
                    f"      CP: {intent_metrics.metrics.context_precision * 100:.1f}% | "
        ...
        output_file = (
            ...
            / f"data/evaluation/ragas_results_gpt120b_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
