...
TYPE_PATTERNS: Dict[str, List[str]] = {
    "LOCATED_IN": [
        ...
        r"\bsituated in\b",
        r"\bin the city of\b",
        r"\bin\b.*\b(city|country|state|region)\b",
    ],
    "WORKS_FOR": [
        r"\bworks for\b",
        r"\bemployed by\b",
    ...
    "TEACHES": [r"\bteaches\b", r"\btrains\b", r"\beducates\b", r"\binstructs\b", r"\bmentors\b"],
    "PRECEDED_BY": [
        r"\bpreceded by\b",
        r"\bafter\b",
        r"\bfollowing\b",
...
async def fetch_relations_with_descriptions(tx, batch_size: int = 100) -> List[Dict]:
    ...
    async for record in result:
        relations.append({"id": record["rel_id"], "description": record["description"]})
    return relations
        ...
        async with driver.session() as session:
            ...
            if not dry_run and updates:
                ...
                for i in range(0, len(updates), batch_size):
                    ...
                    logger.info(
                        f"  Batch {i // batch_size + 1}/{(len(updates) - 1) // batch_size + 1}: {updated} relations updated"
                    )
            ...
            if not dry_run:
                ...
                if total_null_after == 0:
                    ...
                    logger.warning(
                        f"⚠️ {total_null_after} NULL relation_types remain (may need re-run)"
                    )
